...
public class Server {
	...
	static final AudioClip ALERT_AUDIOCLIP = new AudioClip(
			Server.class.getResource("/Music/ringing.wav").toString());
	...
	public void start() {
		...
		try 
		{
			 Server.ALERT_AUDIOCLIP.play();
			 Server.ALERT_AUDIOCLIP.setCycleCount(Server.ALERT_AUDIOCLIP.INDEFINITE);//repeat forever
	...
	public void stop() {
		...
		try {
			...
			new Socket("127.0.0.1", port);//server IP
		}
		catch(Exception e) {
