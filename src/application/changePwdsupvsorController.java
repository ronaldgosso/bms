...
public class changePwdsupvsorController implements Initializable {
	...
	public void change(ActionEvent ev) {
		...
		if(empty() & verify() & space()) {
			try {
				...
				changePwdsupvsorController.ALERT_AUDIOCLIP.play();

				Alert alert = new Alert(AlertType.INFORMATION);
				alert.setTitle("PROMPT");
				alert.setGraphic(new ImageView(this.getClass().getResource("/img/success.jpg").toString()));
				alert.setContentText("Supervisor Password Changed");
...
if(newPassword.getText().equals(verify.getText())) {
	...
	changePwdsupvsorController.ALERT_AUDIOCLIPY.play();
	Alert alert = new Alert(AlertType.ERROR);
	alert.setTitle("PROMPT");
	alert.setGraphic(new ImageView(this.getClass().getResource("/img/Delete-icon.png").toString()));
	...
	public boolean space() {
		...
		if(!space && !space2) {
			...
			alert.setTitle("PROMPT");
			alert.setGraphic(new ImageView(this.getClass().getResource("/img/Delete-icon.png").toString()));
			alert.setContentText("Submisiion of Empty values is PROHIBITED");
			alert.setHeaderText(null);
			Stage stage = (Stage) alert.getDialogPane().getScene().getWindow();
			stage.getIcons().add(new Image(this.getClass().getResource("/img/baga.png").toString()));
			alert.showAndWait();
