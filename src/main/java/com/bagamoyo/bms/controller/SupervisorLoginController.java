...
/**
 * Supervisor login screen controller.
 * Authenticates against the supervisor table using AuthService.validateSupervisor().
...
public class SupervisorLoginController implements Initializable {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SupervisorLoginController.class);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        logger.info("Initializing SupervisorLoginController");
        ...
        logger.debug("SupervisorLoginController initialized");
    }

    /**
     * Handle the supervisor login button action.
     * Authenticates against the supervisor table using BCrypt password verification.
     */
    @FXML
    private void handleLogin() {
        ...
        if (username.isEmpty() || password.isEmpty()) {
            logger.warn("Supervisor login attempt with empty credentials");
            showErrorNotification("Login Failed", "Please enter both username and password.");
            updateStatus("Error: Empty fields");
            return;
        }

        logger.info("Supervisor login attempt for user: {}", username);

        // Update status
        updateStatus("Authenticating...");

        // Authenticate using AuthService with BCrypt verification
        boolean authenticated = authService.validateSupervisor(username, password);

        if (authenticated) {
            logger.info("Successful supervisor login for user: {}", username);
            showSuccessNotification("Login Successful", "Welcome, " + username + "!");
            updateStatus("Success");

            // Navigate to supervisor dashboard
            navigateToSupervisorHome();

            // Clear fields
            if (user != null) user.clear();
            if (pass != null) pass.clear();
        } else {
            logger.warn("Failed supervisor login for user: {}", username);
    ...
    /**
     * Navigate to the supervisor home dashboard.
     */
    private void navigateToSupervisorHome() {
        try {
            ...
            // Load supervisor home dashboard
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/supervisorHome.fxml"));
            Parent root = loader.load();

            Stage dashboardStage = new Stage();
            dashboardStage.setTitle("Supervisor's Panel");
            try {
            ...
            logger.info("Navigated to Supervisor Home dashboard");
        } catch (Exception e) {
            logger.error("Failed to navigate to Supervisor Home dashboard", e);
    ...
    /**
     * Navigate to the admin login screen from the supervisor login.
     */
    @FXML
    private void goToAdminLogin() {
        logger.info("Navigating to admin login from supervisor login");
        try {
            Stage currentStage = getCurrentStage();
            if (currentStage != null) {
                currentStage.close();
            ...
            stage.setTitle("Administration Log In Panel");
            try {
                Image icon = new Image(getClass().getResourceAsStream("/img/baga.png"));
            ...
            logger.error("Failed to navigate to admin login", e);
        }
    }
    ...
    private void goToHome() {
        logger.info("Navigating back to home screen from supervisor login");
        try {
            Stage currentStage = getCurrentStage();
            if (currentStage != null) {
                currentStage.close();
